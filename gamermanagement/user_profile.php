<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_POST['save'])) {
      // Process the form data when the "Edit" button is clicked
      $id = $_POST["id"];
      $email = $_POST["email"];
      ...
      $stmt->close();
    } elseif (isset($_POST['request_elevated_access'])) {
      // Process the request for elevated access
      $user_id = $_SESSION['USER_ID'];
      $request_type = 'Elevated Access User';
  
      // Check if the user already has an elevated access request pending
      $existing_request_query = "SELECT * FROM access_requests WHERE user_id = $user_id AND status = 'Pending'";
      $existing_request_result = $connection->query($existing_request_query);
  
      if ($existing_request_result->num_rows > 0) {
        // If the user already has a pending request, display a message or redirect back to the form
        $accessMessage = "You already have a pending Elevated Access request.";
        // You can also redirect the user back to the form using: header("Location: user_profile.php");
      } else {
        // If the user doesn't have a pending request, insert a new request into the access_requests table
        $insert_request_query = "INSERT INTO access_requests (user_id, request_type, status) VALUES ($user_id, '$request_type', 'Pending')";
  
        if ($connection->query($insert_request_query) === TRUE) {
          $accessMessage = "Elevated Access request sent successfully.";
        } else {
          echo "Error sending Elevated Access request: " . $connection->error;
        }
      }
    }
  }
...
<html lang="en">
  <head>
    ...
    <title>User Profile - Gamer Management</title>
  ...
  <body>
      ...
      <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            <a href="/gamermanagement/user_dashboard.php" class="text-white hover:text-gray-200">Welcome</a>
            <a href="/gamermanagement/user_profile.php" class="text-white hover:text-gray-200">Profile</a>
            <!-- Dropdown Menu -->
            <div class="relative">
    ...
    <h1 class="text-3xl font-semibold text-gray-800 ml-10 mt-4">User Profile</h1>
    ...
    <div class="fixed right-20 top-1/2 transform -translate-y-1/2">
      <form method="post">
        <button type="submit" name="request_elevated_access" class="px-4 py-2 bg-blue-500 text-white font-semibold rounded-md hover:bg-blue-600">
